...
export class VerEdicionComponent {
  ...
  constructor(
    ...
    if (data.isEditMode && data.personaje) {
      ...
      this.newPassword = ''; // Inicializa newPassword como vacío
      this.imagenPreview = this.getImageUrl(this.personaje.id);
    }
  }
  ...
  private addPersonaje(file?: File): void {
    this.personajeService.addPerson(this.personaje, file).subscribe({
      next: (newPersonaje) => {
        this.dialogRef.close();
      },
      error: (err) => {
        console.error('Error adding personaje:', err);
        alert('Error adding personaje. Please try again later.');
      }
    });
  }
  
  private updatePersonaje(file?: File): void {
    // Solo incluye la nueva contraseña si se ha cambiado
    const newPassword = this.personaje.contrasenia; // Obtén la nueva contraseña del input
    if (newPassword) {
      // Envía la nueva contraseña para ser hashada
      this.personajeService.updatePerson(this.personaje.id, this.personaje, file).subscribe({
        next: (updatedPersonaje) => {
          this.dialogRef.close();
        },
        error: (err) => {
          console.error('Error al actualizar el personaje:', err);
          alert('Error al actualizar el personaje. Intenta de nuevo más tarde.');
        }
      });
    } else {
      // Si no se ingresa una nueva contraseña, envía la existente (opcional)
      this.personajeService.updatePerson(this.personaje.id, { ...this.personaje, contrasenia: undefined }, file).subscribe({
        next: (updatedPersonaje) => {
          this.dialogRef.close();
        },
        error: (err) => {
  ...
  onFileChange(event: Event): void {
    ...
    if (input.files && input.files[0]) {
      ...
      reader.onload = (e: any) => {
        ...
        this.isNewImage = true; // Marca que hay una nueva imagen seleccionada
      };
  ...
  private isFormValid(): boolean {
    return this.personaje.contrasenia.trim() !== ''
  }
}
